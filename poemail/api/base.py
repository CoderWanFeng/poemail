...
def send_file(key, msg_from, msg_to, file_path, msg_subject='', content='', host='smtp.qq.com', port=465):
    """
    发送文本邮件
    ...
    e_server = BaseEmail(key=key,
                         ...
                         msg_to=msg_to,
                         msg_subject=msg_subject)
    e_server.send_file(content, file_path)


def send_mail(key, msg_from, msg_to, file_path=None, msg_subject='', content='', host=Mail_Type['qq'], port=465):
    """
    发送邮件函数
    ...
    e_server = BaseEmail(key=key,
                         ...
                         host=host,
                         port=port)
    e_server.send_mail(content, file_path)
